---
title: "Premier League 2023/2024: Tottenham Hotspur FC"
runtime: shiny
output: 
  flexdashboard::flex_dashboard:
    vertical_layout: scroll
    favicon: totenhamhotspurlogo.png
    theme: lumen
    social: [twitter, linkedin]
    

---

```{r ,global_options, include=FALSE}
library(knitr)
opts_chunk$set(message=FALSE, warning=FALSE, tidy.opts = list(width.cutoff=60))
suppressPackageStartupMessages(library(ggthemes))
suppressPackageStartupMessages(library(plotly))
suppressPackageStartupMessages(library(tidyverse))
suppressPackageStartupMessages(library(shiny))
suppressPackageStartupMessages(library(flexdashboard))
suppressPackageStartupMessages(library(pander))
suppressPackageStartupMessages(library(DT))
suppressPackageStartupMessages(library(ggvis))
suppressPackageStartupMessages(library(viridis))
suppressPackageStartupMessages(library(readxl))
suppressPackageStartupMessages(library(ggplotlyExtra))

epl=data.frame(read.csv("matches.csv", header=TRUE))
tot<-epl[epl$Team=="TottenhamHotspur",]
```

Introduction
=============================================================

Column
---------------------------------------------------------------

Tottenham Hotspur is a professional football club based in North London, England. The team took part in the 2023/24 Premier League season and managed a 5th place finish under new manager Ange Postecoglou. Spurs ended up qualifynig for the newly formatted UEFA Europa League 2024/25 campaign. This dashboard looks at the club's on-the-pitch performance in the English top flight division. 

### Ange Postecoglou

![](C:\Users\Admin\Desktop\gift\statistical computing\ange.jpg)



Column
---------------------------------------------------------------
### 

![](C://Users//Admin//Desktop//gift//statistical computing//ths1.jpg)


### 

![](C://Users//Admin//Desktop//gift//statistical computing//IMG_20240829_232937.jpg)



Players {data-orientation="rows"}
===========================================================

Row
------------------------------------------------------------
```{r}
## PLAYERS' DATASET
sheets<-excel_sheets("Soccer-Stats-Premier-League-2023-2024_Final_20240610.xlsx")
players<-data.frame(read_excel("Soccer-Stats-Premier-League-2023-2024_Final_20240610.xlsx",
                               sheet = sheets[4]))

# dim(players) # 746 rows by 27 columns
# Checking for duplicates  
# players[duplicated(players), ]  # No duplicates found
# Checking for missing values
# players[!complete.cases(players), ] # 572 cases with NAs  
#players<-players[complete.cases(players), ]# 487 complete cases
players1<-players[,c(2,7,9,10,12,20,23,24,25,26)]

# DATA TYPE CONVERTION
players1$Name<-factor(players1$Name)
players1$Team<-factor(players1$Team)
players1$Position<-factor(players1$Position)
```

### Registered players

```{r}
players %>% 
  filter(Team == "Tottenham Hotspur") %>% 
  count() %>% 
  valueBox(color="#132257")
```

### Goalkeepers

```{r}
players %>% 
  filter(Team == "Tottenham Hotspur",
         Position == "Goalkeeper") %>% 
  count() %>% 
  valueBox(color="#131F53")
```

### Defenders

```{r}
players %>% 
  filter(Team == "Tottenham Hotspur",
         Position == "Defender") %>% 
  count() %>% 
  valueBox(color = "#131F53")
```

### Midfielders

```{r}
players %>% 
  filter(Team == "Tottenham Hotspur",
         Position == "Midfielder") %>% 
  count() %>% 
  valueBox(color="#131F53")
```

### Forwards

```{r}
players %>% 
  filter(Team == "Tottenham Hotspur",
         Position == "Forward") %>% 
  count() %>% 
  valueBox(color="#131F53")
```

### Average age (years)

```{r}
players %>% 
  filter(Team == "Tottenham Hotspur") %>% 
  summarise(round(mean(Age),0)) %>% 
  valueBox(color="#131F53")
```

Row {.tabset .tabset-fade}
----------------------------------------------------------------

### Goalkeepers

```{r}
players %>% 
  filter(Team == "Tottenham Hotspur",
         Position == "Goalkeeper") %>% 
  select(Citizenship, Name, Jersey) %>% 
  pander()
```

### Defenders

```{r}
players %>% 
  filter(Team == "Tottenham Hotspur",
         Position == "Defender") %>% 
  select(Citizenship, Name, Jersey) %>% 
  pander()
```

### Midfielders

```{r}
players %>% 
  filter(Team == "Tottenham Hotspur",
         Position == "Midfielder") %>% 
  select(Citizenship, Name, Jersey) %>% 
  pander()
```

### Forwards

```{r}
players %>% 
  filter(Team == "Tottenham Hotspur",
         Position == "Forward") %>% 
  select(Citizenship, Name, Jersey) %>% 
  pander()
```


Outcomes
=========================================================

Column
-------------------------------------------------------
### Home


```{r}
cat("Wins","[%]")
#tot%>% 
  #filter(Venue == "Home" & Result=="W") %>% 
  #summarise(win.pct.h=n()*100/19) %>% 
  gauge(68.42, min=0,max=100,symbol="%",
        gaugeSectors(success = c(80,100),
                     warning =c(40,79),
                     danger = c(0,39)))

```

### 

```{r}
cat("Draws","[%]")
#tot %>% 
  #filter(Venue == "Home" & Result=="D") %>% 
  #summarise(draw.pct.h=n()*100/19) %>% 
  gauge(0, min=0,max=100,symbol="%")

```

### 

```{r}
cat("Losses","[%]")
#tot %>% 
  #filter(Venue == "Home" & Result=="L") %>% 
  #summarise(loss.pct.h=n()*100/19) %>% 
  gauge(31.58, min=0,max=100,symbol="%",
        gaugeSectors(success = c(0,39),
                     warning =c(40,79),
                     danger = c(80,100)))

```

Column
-------------------------------------------------------
### Away


```{r}
cat("Wins","[%]")
#tot %>% 
  #filter(Venue == "Away" & Result=="W") %>% 
  #summarise(win.pct.a=n()*100/19) %>% 
  gauge(36.84, min=0,max=100,symbol="%",
        gaugeSectors(success = c(80,100),
                     warning =c(40,79),
                     danger = c(0,39)))

```

### 

```{r}
cat("Draws","[%]")
#tot %>% 
  #filter(Venue == "Away" & Result=="D") %>% 
  #summarise(draw.pct.a=n()*100/19) %>% 
  gauge(31.58, min=0,max=100,symbol="%")

```

### 

```{r}
cat("Losses","[%]")
#tot %>% 
  #filter(Venue == "Away" & Result=="L") %>% 
  #summarise(loss.pct.a=n()*100/19) %>% 
  gauge(31.58, min=0,max=100,symbol="%",
        gaugeSectors(success = c(0,39),
                     warning =c(40,79),
                     danger = c(80,100)))

```

Awards
===========================================================

Column
------------------------------------------------------------
### Goals scored

```{r, fig.align="top", out.width="10%"}
include_graphics(
"C://Users//Admin//Desktop//gift//statistical computing//son.jpg")
```

### 
```{r}
players1 %>%
  filter(Team == "Tottenham Hotspur",
         totalGoals >= 1) %>% 
  select(Name, totalGoals) %>%
  arrange(desc(totalGoals)) %>% 
  pander()
  
```

Column
------------------------------------------------------------
### Goal Assists

```{r, fig.align="top", out.width="10%"}
include_graphics(
"C://Users//Admin//Desktop//gift//statistical computing//johnson.jpg")
```

###
```{r}
players1 %>%
  filter(Team == "Tottenham Hotspur",
         goalAssists >= 1) %>% 
  select(Name, goalAssists) %>%
  arrange(desc(goalAssists)) %>% 
  pander()
  
```


Totenham Hotspur Stadium
===========================================================

Column 
-----------------------------------------------------------
### 

```{r, fig.align="top", out.width="40%"}
include_graphics(
"C://Users//Admin//Desktop//gift//statistical computing//ths4.jpg")
```

### 

```{r, fig.align="bottom", out.width="20%"}
include_graphics(
"C://Users//Admin//Desktop//gift//statistical computing//ths3.jpg")
```


Column{.tabset .tabset-fade}
----------------------------------------------------------------------

### Notes

Tottenham Hotspur Stadium played host to 19 Premier League home games over the 2023/24 campaign. The average match attendance stands at **61,482**.

The highest attendance was recorded in **September 2023** against **Liverpool** (full attendance) and the lowest at **61,065** against **Manchester City** in **May of 2024**.


### Home matches

```{r}
tot.home<-
  tot[tot$Venue == "Home",]%>% 
  select(c(Round,Opponent,Attendance)) %>% 
  arrange(desc(Attendance)) %>%
  mutate(percent =   round(Attendance*100/(62001),2))%>% 
  datatable(rownames=FALSE); tot.home  
```

### Distribution

```{r}
tot.at.home<-tot[tot$Venue=="Home", ]
tot.attendance<-ggplot(tot.at.home, aes(x=Date,y=Attendance))+
  geom_point(shape=20, size=4, color="#131F53", alpha=0.85)+
  geom_hline(aes(yintercept=round(mean(Attendance),0)))+
  theme(axis.text.x = element_text(angle=45, hjust=1))+
  labs(x="Time",
       y="Attendance",
       title="Premier League\nhome attendance:TH Stadium 2023/24")+
  theme_pander()

plotly::ggplotly(p=tot.attendance)
```


Goals scored {data-navmenu="Offensive record"}
======================================================================

```{r , fig.width=14, fig.height=5}
offensive1<-epl %>% 
  group_by(Team) %>% 
  summarise(goals_scored = sum(GF)) %>% 
  arrange(desc(goals_scored)) %>% 
  head(10)  

GS<-ggplot(offensive1,aes(x=reorder(Team, goals_scored), y=goals_scored))+
  geom_bar(stat='identity',fill = "#131F53", alpha=0.85)+
  theme(axis.text.x = element_text(angle=45, hjust=1))+
 labs(x="Teams",
      y="Goals scored",
      title="Goals scored:\nPremier League 2023/2024",
      subtitle = "Top 10")+
  theme_pander()

ggplotly(GS)
```

Column
----------------------------------------------------------------------

### Position

```{r,fig.width=4}
epl %>% 
  group_by(Team) %>% 
  summarise(total.goals = sum(GF)) %>%
  arrange(desc(total.goals)) %>% 
  mutate(position = min_rank(-total.goals)) %>% 
  filter(Team == "TottenhamHotspur") %>% 
  select(position) %>%
valueBox(color="#131F53")
```

### Goals scored

```{r,fig.width=4}
valueBox(value=sum(tot$GF),color="#131F53")
```


Column
-------------------------------------------------------------------------

### Home Goals

```{r, fig.width=4}

valueBox(value = sum(tot.at.home$GF),color="#131F53")
```

### Away Goals

```{r, fig.width=4}
valueBox(value=sum(tot$GF)-sum(tot.at.home$GF),color="#131F53")
```



# Shots {data-navmenu="Offensive record"}

```{r ,fig.width=14, fig.height=5}
offensive2<-epl %>% 
  group_by(Team) %>%
  summarise(Shot_accuracy = sum(SoT)*100/sum(Sh)) %>% 
  arrange(desc(Shot_accuracy)) %>% 
  head(10) 

SOT<-ggplot(offensive2 ,aes(x=reorder(Team, Shot_accuracy), y=Shot_accuracy))+
  geom_bar(stat='identity',fill = "#131F53", alpha = 0.85)+
  theme_pander()+
  theme(axis.text.x = element_text(angle=45, hjust=1))+
  labs(x="Teams",
       y="Percentage",
       title="Shot accuracy [%]:\nPremier League 2023/2024",
       subtitle = "Top 10")

ggplotly(SOT)
```

Column
----------------------------------------------------------------------

### Position

```{r,fig.width=4}
epl %>% 
  group_by(Team) %>% 
  summarise(shot.acc = sum(SoT)*100/sum(Sh)) %>%
  arrange(desc(shot.acc)) %>% 
  mutate(position = min_rank(-shot.acc)) %>% 
  filter(Team == "TottenhamHotspur") %>% 
  select(position) %>%
valueBox(color="#131F53")
```

### Shot accuracy [%]

```{r,fig.width=4}
valueBox(value=round(sum(tot$SoT)*100/sum(tot$Sh), 2)
, color="#131F53")
```


Possession {data-navmenu="Offensive record"}
========================================================

```{r ,fig.width=14, fig.height=5}
offensive3<-epl %>% 
  group_by(Team) %>%
  summarise(avg.poss = mean(Poss)) %>% 
  arrange(desc(avg.poss)) %>% 
  head(10)  

POSS<-ggplot(offensive3, aes(x=reorder(Team, avg.poss), y=avg.poss))+
  geom_bar(stat='identity',fill = "#131F53", alpha=0.85)+
  theme_pander()+
  theme(axis.text.x = element_text(angle=45, hjust=1))+
  labs(x="Teams",
       y="Percentage",
       title="Average ball possession [%]:\nPremier League 2023/2024",
       subtitle = "Top 10")

ggplotly(POSS)
```

Column
----------------------------------------------------------------------

### Position

```{r,fig.width=4}
epl %>% 
  group_by(Team) %>% 
  summarise(avg.poss = mean(Poss)) %>%
  arrange(desc(avg.poss)) %>% 
  mutate(position = min_rank(-avg.poss)) %>% 
  filter(Team == "TottenhamHotspur") %>% 
  select(position) %>%
valueBox(color="#131F53")
```

### Average possesion [%]

```{r,fig.width=4}
valueBox(value=round(mean(tot$Poss), 2)
, color="#131F53")
```

# Goals conceded {data-navmenu="Defensive record"}

```{r ,fig.width=14, fig.height=5}
defensive1<-epl %>% 
  group_by(Team) %>%
  summarise(gc = sum(GA)) %>% 
  arrange(gc) %>% 
  head(10) 
CONC<-ggplot(defensive1, aes(x=reorder(Team, gc), y=gc))+
  geom_bar(stat='identity', fill = "#131F53", alpha=0.85)+
  theme_pander()+
  theme(axis.text.x = element_text(angle=45, hjust=1))+
  labs(x="Teams",
       y="goals",
       title="Goals conceded:\nPremier League 2023/2024",
       subtitle = "Top 10:Least conceded goals")

ggplotly(CONC)
```

Column
----------------------------------------------------------------------

### Position

```{r,fig.width=4}
epl %>% 
  group_by(Team) %>% 
  summarise(g.c = sum(GA)) %>%
  arrange(desc(g.c)) %>% 
  mutate(position = min_rank(g.c)) %>% 
  filter(Team == "TottenhamHotspur") %>% 
  select(position) %>%
valueBox(color="#131F53")
```

### Goals conceded

```{r,fig.width=4}
valueBox(value=sum(tot$GA), color="#131F53")
```

Column
----------------------------------------------------------------------

### At Home

```{r, fig.width=4}

valueBox(value = sum(tot.at.home$GA), 
         color="#131F53")
```

### Away

```{r, fig.width=4}
valueBox(value=sum(tot$GA)-sum(tot.at.home$GA), 
         color="#131F53")
```

# Clean sheets {data-navmenu="Defensive record"}

```{r,fig.width=14, fig.height=5}
defensive2<-epl[epl$GA == 0, ] %>% 
  group_by(Team) %>% 
  count() %>% 
  arrange(desc(n)) %>% 
  head(20)
  
CS<-ggplot(defensive2,aes(x=reorder(Team, n), y=n))+
  geom_bar(stat='identity', fill = "#131F53", alpha=0.85)+
  theme_pander()+
  theme(axis.text.x = element_text(angle=45, hjust=1))+
  labs(x="Teams",
       y="Frequency",
       title="Clean sheets:\nPremier League 2023/2024") 

ggplotly(CS)
```

Column
----------------------------------------------------------------------

### Position

```{r}
epl[epl$GA == 0, ] %>% 
  group_by(Team) %>% 
  summarise(c.s = n()) %>%
  mutate(position = min_rank(-c.s)) %>%
  arrange(desc(c.s)) %>% 
  filter(Team == "TottenhamHotspur") %>% 
  select(position) %>%
valueBox(color="#131F53")
```

### Clean sheets

```{r}
tot[tot$GA==0,] %>% 
  count() %>% 
  valueBox(color="#131F53")
```

Column
----------------------------------------------------------------------

### At home

```{r}
tot[tot$Venue == "Home",] %>%
  filter(GA == 0) %>% 
  nrow() %>% 
valueBox(color="#131F53")
```

### Away

```{r}
# 7-2=5
tot[tot$Venue == "Away",] %>%
  filter(GA == 0) %>% 
  nrow() %>% 
valueBox(color="#131F53")
```


References
============================================================

- [www.kaggle.com/datasets/mertbayraktar/english-premier-league-matches-20232024-season](www.kaggle.com/datasets/mertbayraktar/english-premier-league-matches-20232024-season)

- [https://pkgs.rstudio.com/flexdashboard/](https://pkgs.rstudio.com/flexdashboard/)

- [https://www.excel4soccer.com/
](https://www.excel4soccer.com/
)
